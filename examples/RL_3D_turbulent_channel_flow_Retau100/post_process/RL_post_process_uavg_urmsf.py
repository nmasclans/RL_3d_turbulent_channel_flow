...
for i_RL in range(n_RL):
    L2_error_avg_u_plus_RL[i_RL]  = np.sqrt( np.sum( ( avg_u_plus_RL[i_RL,:]  - avg_u_plus_ref )**2 ) )
    L2_error_rmsf_u_plus_RL[i_RL] = np.sqrt( np.sum( ( rmsf_u_plus_RL[i_RL,:] - rmsf_u_plus_ref )**2 ) )
    L2_error_rmsf_v_plus_RL[i_RL] = np.sqrt( np.sum( ( rmsf_v_plus_RL[i_RL,:] - rmsf_v_plus_ref )**2 ) )
    L2_error_rmsf_w_plus_RL[i_RL] = np.sqrt( np.sum( ( rmsf_w_plus_RL[i_RL,:] - rmsf_w_plus_ref )**2 ) )
L2_error_avg_u_plus_nonRL  = np.sqrt( np.sum( (avg_u_plus_nonRL  - avg_u_plus_ref)**2 ) )
L2_error_rmsf_u_plus_nonRL = np.sqrt( np.sum( (rmsf_u_plus_nonRL - rmsf_u_plus_ref)**2 ) )
L2_error_rmsf_v_plus_nonRL = np.sqrt( np.sum( (rmsf_v_plus_nonRL - rmsf_v_plus_ref)**2 ) )
L2_error_rmsf_w_plus_nonRL = np.sqrt( np.sum( (rmsf_w_plus_nonRL - rmsf_w_plus_ref)**2 ) )
