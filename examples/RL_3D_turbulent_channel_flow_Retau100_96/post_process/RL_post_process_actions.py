...
try :
    ...
    raise ValueError("Missing call arguments, should be: <ensemble> <train_name>")
...
verbose = False
action_data_dir = f"../train/{train_name}/action/"
time_data_dir   = f"../train/{train_name}/time/"
...
frames_dict_pdf = {}
for i_RL in range(n_RL):
    frames_dict_scatter[i_RL] = []
    frames_dict_pdf[i_RL] = []
for i_RL in range(n_RL):
    # log progress
    if i_RL % (n_RL//10 or 1) == 0:
        print(f"{i_RL/n_RL*100:.0f}%")
    # Build frames
    frames_dict_scatter, frames_dict_pdf, action_dim = visualizer.build_actions_frames(frames_dict_scatter, frames_dict_pdf, avg_time_data, action_data[i_RL], global_step_list[i_RL], ylim)
