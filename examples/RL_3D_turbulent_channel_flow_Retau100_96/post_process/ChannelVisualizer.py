...
# --------------------- u-velocity vs reference as gif evolution ------------------------
    ...
    def build_um_fig(self, yplus_odt, yplus_ref, um_odt, um_ref, avg_time, global_step, ylim=None):
        
        fig, ax = plt.subplots()
        plt.semilogx(yplus_ref, um_ref, '--', color="black", lw=2, label=r"Reference")
        plt.semilogx(yplus_odt, um_odt, '-',  color="black", lw=2, label=r"RL")
    ...
    def build_um_frame(self, frames, yplus_odt, yplus_ref, um_odt, um_ref, avg_time, global_step, ylim=None):
        
        fig = self.build_um_fig(yplus_odt, yplus_ref, um_odt, um_ref, avg_time, global_step, ylim)
        fig.canvas.draw()
    ...
    def build_urmsf_fig(self, yplus_odt, yplus_ref, urmsf_odt, urmsf_ref, avg_time, global_step, ylim=None):
    ...
    def build_urmsf_frame(self, frames, yplus_odt, yplus_ref, urmsf_odt, urmsf_ref, avg_time, global_step, ylim=None):
        
        fig = self.build_urmsf_fig(yplus_odt, yplus_ref, urmsf_odt, urmsf_ref, avg_time, global_step, ylim)
        fig.canvas.draw()
        img = Image.frombytes("RGB", fig.canvas.get_width_height(), fig.canvas.tostring_rgb())
        frames.append(img)
        plt.close()
...
# ------------------- multiple-dimensions RL actions  -------------------
    ...
    def build_actions_scatter(self, avg_time, actions, global_step, ylim):
        """
    ...
    def build_actions_pdf(self, actions, global_step):
        """
        avg_time: np.array([num_time_steps])
        actions:  np.array([num_time_steps, action_dim, rl_n_envs])
        """
    ...
    def build_actions_frames(self, frames_dict_scatter, frames_dict_pdf, avg_time, actions, global_step, ylim):
        ...
        return frames_dict_scatter, frames_dict_pdf, action_dim
