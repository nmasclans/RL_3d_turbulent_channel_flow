...
# --- Script parameters ---
verbose = False
output_data_dir = "../rhea_exp/output_data/"
...
filename_ref = "reference_data/3d_turbulent_channel_flow_reference.h5"
...
assert averaging_time_ref > averaging_time_nonConv, f"Reference data averaging time {averaging_time_ref} must be greater than non-converged averaging time {averaging_time_nonConv}"
...
for i_RL in range(n_RL):
    ...
    frames_rmsf_u      = visualizer.build_urmsf_frame(frames_rmsf_u, y_delta_RL, y_delta_ref, rmsf_u_RL[i_RL], rmsf_u_ref, averaging_time_nonConv, ylim=[0.0, rmsf_u_max])
    frames_rkk         = visualizer.build_reynolds_stress_tensor_trace_frame( frames_rkk, y_delta_RL, y_delta_ref, Rkk_RL[i_RL],    Rkk_ref, averaging_time_nonConv)
    frames_eig         = visualizer.build_anisotropy_tensor_eigenvalues_frame(frames_eig, y_delta_RL, y_delta_ref, eigval_RL[i_RL], eigval_ref, averaging_time_nonConv)
    frames_xmap_coord  = visualizer.build_anisotropy_tensor_barycentric_xmap_coord_frame( frames_xmap_coord,  y_delta_RL, y_delta_ref, xmap1_RL[i_RL], xmap2_RL[i_RL], xmap1_ref, xmap2_ref, averaging_time_nonConv)
    frames_xmap_triang = visualizer.build_anisotropy_tensor_barycentric_xmap_triang_frame(frames_xmap_triang, y_delta_RL, xmap1_RL[i_RL], xmap2_RL[i_RL], averaging_time_nonConv)
