...
# ------------------------------------------------------------------------
    ...
    def build_rewards_terms_plot(self, avg_time, local_reward, avg_u_reward, avg_v_reward, avg_w_reward, rms_u_reward, rms_v_reward, rms_w_reward, avg_time_lim, rewards_lim, global_step):
        ...
        ax[1].plot(avg_time, avg_v_reward, linewidth=2, label=r"Reward term $\overline{v}$")
        ax[1].plot(avg_time, avg_w_reward, linewidth=2, label=r"Reward term $\overline{w}$")
        ...
        for axis in range(2):
            ax[axis].set_xlim(avg_time_lim)
            ax[axis].grid(axis="y")
            ax[axis].legend(loc='center left', bbox_to_anchor=(1, 0.5))
        fig.suptitle(f"RL step = {global_step}", fontsize=16)
        plt.tight_layout()
        fig_plot = fig
        plt.close()
        return fig_plot
